...
func TestUpdatePowerZero(t *testing.T) {
	...
	for _, tc := range tc {
		...
		lp.Mode = tc.mode
		lp.Update(0, false, false, 0) // sitePower 0
	...
	for _, status := range []api.ChargeStatus{api.StatusB, api.StatusC} {
		for _, tc := range tc {
			...
			for step, se := range tc.series {
				...
				current := lp.pvMaxCurrent(api.ModePV, se.site, false, 0)
...
func TestPVHysteresisForStatusOtherThanC(t *testing.T) {
	...
	current := lp.pvMaxCurrent(api.ModePV, sitePower, false, 0)
